...
class SinglyLinkedList {
    ...
    reverseInPlace() {
        ...
        end gross O(n^2) solve*/

        // begin O(n) solve
        let curr = this.head;
        let [next, prev] = [null, null];
        do {
            next = curr.next; // reassign next pointer to current node's next
            curr.next = prev; // reassign current node's next pointer to prev
            prev = curr;      // reassign prev to curr so that we can build backwards
            curr = next;      // reassign curr to next so that next iteration makes sense
        } while(next);
        this.head = prev;     // update LL's head pointer to prev, which should be what was the tail once our loop ends
        // Write your hypothesis on the time complexity of this method here
    }
